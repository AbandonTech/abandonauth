...
| `src/api/cmd/abandonauth.go` | commands, flags, environment binding, and Swagger metadata; the secrets are read from the environment only and have no flag |
| `src/api/cmd/operations.go` | `serve`, `maintenance`, `database rotate-auth-epoch` |
| `src/api/internal/config/` | validated immutable configuration; `PasswordSignInEnabled` decides whether that build may serve them |
...
| `src/api/internal/urlpolicy/` | callback URI policy and origin parsing; `Callback` keeps the registered spelling a browser is returned to alongside the parsed form the rules are applied to |

### Database

| Path | Holds |
| --- | --- |
| `internal/database/migrations/` | embedded goose migrations: the baseline schema and its `schema_identity` marker, then auth state, sessions, revocation, and rate limits |
...
| `internal/database/migrate.go` | the embedded migration runner, which inspects and migrates under one advisory lock |
| `internal/database/schemastate.go` | what a database looks like before anything is done to it, and which states may be migrated |
...
The deployment image is what is published. It carries no password sign-in,
refuses `DEBUG=true`, runs as an account that is not root, and holds nothing but
the binary and a certificate bundle. It does carry the documentation and the
schema, which every build serves. `compose.yml` builds it unless
`API_BUILD_TARGET` in `.env` names the `development` target, which is the
variant the password routes are compiled into. Both images are built on every
pull request by `.github/workflows/`.
...
| `src/api/README.md` | the API's own commands, checks, codegen, migrations, and builds |
